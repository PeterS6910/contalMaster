...
{
    [LwSerialize(725)]
    public class DcuOfflineAlarm : Alarm, ICreateServerAlarm
    {
        private DcuOfflineAlarm()
        {
        ...
        public ServerAlarm CreateServerAlarm(Guid idCcu)
        {
            ...
            var dcu = DCUs.Singleton.GetById(AlarmKey.AlarmObject.Id);

            if (dcu == null)
                return null;

            return new DcuOfflineServerAlarm(
                ccu,
                dcu,
                new Alarm(
                    Id,
