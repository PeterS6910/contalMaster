...
namespace Contal.IwQuick.Sys
{
    ...
    public class PatchedFileStream : FileStream
    {
        ...
        public int LowLevelRetryCount
        {
            ...
            set {
                _lowLevelRetryCount = value > 0 ? value : DEFAULT_LOW_LEVEL_RETRY_COUNT;
            }
        }
                        ...
                        if (null == _fileOpenErrorFiltering)
                            lock (_foefLock)
                            {
                                if (null == _fileOpenErrorFiltering)
                                {
                                     ...
                                     tmp.ItemTimedOut += OnErrorItemTimedOut;
        ...
        static void OnErrorItemTimedOut(string key, FileOperation value, int timeout)
        {
            if (ErrorOccured != null)
                try
                {
...
#if DEBUG
                        if (sizeToWrite != (int) (base.Position - originalPosition))
                            // this can be still a legitimate situation
                            // because PatchedFileStream is not synchronized
                            // but to see the probability/occurence
                            DebugHelper.TryBreak("PatchedFileStream.Write statistical break");
#endif
                    }
            ...
            for (int retry = 0; retry < _lowLevelRetryCount; retry++)
            {
                try
                {
                    ...
                    do
                    {
                        ...
                        {
                            DebugHelper.TryBreak("PatchedFileStream.Read in race loop");
                            // should be different than IOException

                            string tmp = "Reading " + Name + " seems to be in race loop";
