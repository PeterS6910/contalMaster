...
// ReSharper disable once TooWideLocalVariableScope
            ...
            if (null != testSocket)
            {
                try
                {
                    try
                    {
                        if (testSocket.LingerState.Enabled)
                            testSocket.LingerState = new LingerOption(false, 0);
                    }
                    catch { }
                    testSocket.Close();
                }
        ...
        /// <exception cref="InvalidTransportPortException">if the specified port is invalid</exception>
        public static bool IsFreeTcp(
            int port, 
            [CanBeNull] IPAddress ipAddress)
        {
            CheckValidity(port);
...
// ReSharper disable once TooWideLocalVariableScope
            ...
            try
            {
                ...
                testSocket.Bind(new IPEndPoint(ipAddress??IPAddress.Any, port));
        ...
        /// <param name="isLocalOnly">if true, the binding is checked on loopback interface, otherwise on 0.0.0.0</param>
        /// <exception cref="InvalidTransportPortException">if the specified port is invalid</exception>
