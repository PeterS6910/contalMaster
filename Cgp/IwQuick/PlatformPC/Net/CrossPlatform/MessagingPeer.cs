...
#region Binary protocol definitions
        ...
        internal class LowLevelMessage
        {
            ...
            internal ByteDataCarrier PrepareHeader(int optionalDataLength)
            {
                ...
                int asPos = rawHeader.ActualSize;
                ...
                out int finishedPosition)
            {
                ...
                byte protoVersion = data[finishedPosition];
                ...
                byte command = data[finishedPosition + PROTO_PART_LENGTH];
            ...
            internal static LowLevelMessage ParseRawData(ByteDataCarrier data)
            {
                ...
                LowLevelMessage llm = ParseRawData(data.Buffer, data.Offset, data.ActualSize, out finishedPosition);
        ...
        internal class LLKeepAliveMessage : LowLevelMessage
        {
                ...
                : base(LowLevelCommand.KeepAlive,false)
            {
                ...
                {
                    int m2aCount = (rawParams.ActualSize/ConnectionInfo.MID2ACK_SIZEOF);
...
// ReSharper disable once UnusedMember.Local
            ...
            internal override ByteDataCarrier GetRawData()
            {
                int odl = _rawParameters.ActualSize;
...
// ReSharper disable once MemberCanBePrivate.Local
            ...
            internal override ByteDataCarrier GetRawData()
            {
                ...
                byte[] amtBytes = BitConverter.GetBytes(_applicationMessageType);

                headerBdc.Append(amtBytes,true);
                headerBdc.Append(_rawParameters,true);
        ...
        public void RegisterApplicationMessageType(IApplicationMessageFactory applicationMessageFactory)
        {
            Validator.CheckNull(applicationMessageFactory);
        ...
        public void Start(IPAddress bindToIp, int peerPort)
        {
            ...
            if (newlyCreated)
            {
                ...
                _sendingPq = new ProcessingQueue<MPDataCarrier>(
                    PQMode.Synchronous,
                    new object[]{},
        ...
        private void ValidateDataCarrierRetry(ConnectionInfo connectionInfo, MPDataCarrier dataCarrier,bool calledViaPollingMaintenanceThread)
        {
            ...
            TimeSpan timeFrom1stProcessing = now - dataCarrier._1stProcessingTimeStamp;

            TimeSpan timeFromLastProcessing = now - dataCarrier._lastProcessingTimeStamp;
            ...
            switch (dataCarrier._lowLevelMessage.Command)
            {
                case LowLevelCommand.BinaryMessage:
                    ...
                    if (BinaryMessageRetryingFailed != null)
                        try
                        {
                            LLBinaryMessage llbm = (LLBinaryMessage) dataCarrier._lowLevelMessage;
                ...
                case LowLevelCommand.TextMessage:
                    if (TextMessageRetryingFailed != null)
                        try
                        {
                            LLTextMessage lltm = (LLTextMessage) dataCarrier._lowLevelMessage;
                ...
                case LowLevelCommand.ApplicationMessage:
                    if (ApplicationMessageRetryingFailed != null)
                    {
                        try
                        {
                            LLApplicationMessage llam = (LLApplicationMessage) dataCarrier._lowLevelMessage;
        ...
        void OnSendProcessing(MPDataCarrier dataCarrier)
        {
            ...
            ConnectionInfo ci = GetConnection(dataCarrier._peerIPe);
                ...
                (ci._client !=null && ci._connection == null))
            {
                ...
                bool connectedYet = ci._client.WaitForConnect(WAITING_FOR_CONNECT_TIMEOUT);
            ...
            if (ci._connection != null)
            {
                ...
                try
                {
                    ByteDataCarrier bdc = dataCarrier._lowLevelMessage.GetRawData();
...
// ReSharper disable once NotAccessedField.Local
                ...
                params uint[] messageIds)
            {
                ...
                foreach (uint mid in messageIds)
                {
            ...
            internal uint[] IncomingMessageIdsToAckSnapshot
            {
                get
                {
                    lock (_incomingMessageIdsToAck)
                    {
                        ...
                        int countToCopy = _incomingMessageIdsToAck.Count;
                        uint[] output;
                        ...
                        if (countToCopy > MID2ACK_MAX_COUNT)
                        {
                            ...
                            LinkedListNode<uint> lln = _incomingMessageIdsToAck.First;
                            int i = 0;
        ...
        private void AddRemovePeerConnection(ISimpleTcpConnection connection, bool add)
        {
            ...
            if (add)
            {
                IPEndPoint ipe = connection.RemoteEndPoint;
...
// ReSharper disable once ImplicitlyCapturedClosure
                    ipePassed =>
                    {
                        ...
                        bool queueAddedAsNew = _sendingPq.EnsureQueue(connection.RemoteEndPoint.Address);
                        DebugHelper.NOP(queueAddedAsNew);
            ...
            else
            {
                _peerConnections.Remove(
                    ...
                    (keyPassed, removed, value) =>
                    {
                        if (removed)
                        {
                            bool removedQueue = _sendingPq.RemoveQueue(keyPassed);
                            if (!removedQueue)
        ...
        private void AddRemovePeerConnection(ConnectionInfo connectionInfo, bool add)
        {
            ...
            if (add)
            {
                ...
                _peerConnections.GetOrAddValue(
                    ...
                    keyPassed =>
                    {
                        bool queueAddedAsNew = _sendingPq.EnsureQueue(connectionInfo._peerIPe.Address);
                        DebugHelper.NOP(queueAddedAsNew);
        ...
        {
            ConnectionInfo ci = GetConnection(connection.RemoteEndPoint);
            if (ci != null)
            ...
            //    // should not happen
            //    Debugger.Break();
            //    DebugHelper.NOP(connection, data);
            //}
#endif
        }
        ...
        private void OnDataReceived(ConnectionInfo connectionInfo, ByteDataCarrier data)
        {
                ...
                raceCondition++ < 64)
            {
                ...
                bool timeoutsInPast = connectionInfo.NullMessageTimeoutCount();
                ...
                {
                    // do this asap
                    LLKeepAliveMessage llkam = (LLKeepAliveMessage) llm;
            ...
            switch (llm.Command)
            {
                case LowLevelCommand.BinaryMessage:
                    if (BinaryMessageReceived != null)
                        try
                        {
                            LLBinaryMessage llbm = (LLBinaryMessage) llm;
                ...
                case LowLevelCommand.TextMessage:
                    if (TextMessageReceived != null)
                        try
                        {
                            LLTextMessage lltm = (LLTextMessage) llm;
                ...
                case LowLevelCommand.ApplicationMessage:
                    if (ApplicationMessageReceived != null)
                        try
                        {
                            LLApplicationMessage llam = (LLApplicationMessage) llm;
        ...
        public void SendMessage(IPAddress peerIp, ByteDataCarrier message, int secondsTimeout,bool retryDuringTimeout)
        {
            ...
            LLBinaryMessage llbm = new LLBinaryMessage(false,message);
...
// ReSharper disable once FunctionNeverReturns
        ...
        {
            IPEndPoint ipe = new IPEndPoint(peerIp, _peerPort);

            ConnectionInfo ci = GetConnection(ipe);
            if (ci == null)
                return false;
        ...
        {
            IPEndPoint ipe = (IPEndPoint)itemToCompareWith;
