...
#endif
{
    ...
    public class DataChannelPeer
    {
        ...
        public class TransferInfo
        {
            ...
            internal TransferInfo(TransferSide side)
            {
                _id = ++_idCounter;
            ...
            /// 
            /// </summary>
            public readonly TransferSide Side;
            ...
            internal SafeThread<TransferInfo,ISimpleTcpConnection> _transferThread = null;

            internal SafeThread<TransferInfo> _progressingThread = null;
                ...
                if (!_stopTransfer)
                    lock (_stopTransferLock)
                        if (!_stopTransfer)
                        {
                            ...
                            try
                            {
                                bool endWithoutTimeout = _waitForEndMutex.WaitOne(2000, false);

                                if (!endWithoutTimeout &&
                                    null != _transferThread &&
                                    _result < 0) // double checking
                                    _transferThread.Stop(2000);
                            }
                            catch
                            {
                            }
            ...
            public void AsyncStopTransfer()
            {
                if (_result < 0 &&
                    !_stopTransfer)
                    SafeThread.StartThread(StopTransfer);
            }
            ...
            {
                string tmp =
                    string.Format("{0};{1}", localIPEndPoint != null ? localIPEndPoint.ToString() : String.Empty,
        ...
        private readonly SyncDictionary<string, TransferInfo> _peers = new SyncDictionary<string,TransferInfo>(8);
...
// ReSharper disable once EmptyConstructor
        ...
        internal class LowLevelMessage
        {
            ...
            {
                ByteDataCarrier rawHeader = new ByteDataCarrier(PREAMBLE.Length + HEADER_LENGTH, false);
            ...
            {
                int pos = ByteDataCarrier.IndexOf(data, 0, length, PREAMBLE);
                if (pos < 0)
                    return null;
                ...
                ByteDataCarrier rawParams = new ByteDataCarrier(data, pos+HEADER_LENGTH, odl);
        ...
        private class LLStoreFileMessage : LowLevelMessage
        {
            ...
            internal LLStoreFileMessage(ByteDataCarrier rawParams)
                :base(LowLevelCommand.StoreFile)
            {
            ...
            internal override ByteDataCarrier GetRawData()
            {
                ...
                ByteDataCarrier headerBdc = PrepareHeader(odl);
        ...
        private class LLPullFileMessage : LowLevelMessage
        {
            ...
            internal LLPullFileMessage(ByteDataCarrier rawParams)
                : base(LowLevelCommand.PullFile)
            {
                _rawParameters = rawParams;
        ...
        private class LLStoreMemoryStreamMessage : LowLevelMessage
        {
            ...
            internal LLStoreMemoryStreamMessage(ByteDataCarrier rawParams)
                : base(LowLevelCommand.StoreMemoryStream)
            {
                _rawParameters = rawParams;
        ...
        private sealed class LLPushDataMessage : LowLevelMessage
        {
            ...
            internal LLPushDataMessage(ByteDataCarrier rawParams)
                : base(LowLevelCommand.PushData)
            {
                _rawParameters = rawParams;
        ...
        private class LLAckNackBaseMessage : LowLevelMessage
        {
            ...
            protected LLAckNackBaseMessage(LowLevelCommand lowLevelCommand, ByteDataCarrier rawParams)
                :base(lowLevelCommand)
            {
                _rawParameters = rawParams;
        ...
        private class LLNotAcknowledgedMessage : LLAckNackBaseMessage
        {
            ...
            internal LLNotAcknowledgedMessage(ByteDataCarrier rawParams)
        ...
        private class LLAcknowledgedMessage : LLAckNackBaseMessage
        {
            ...
            internal LLAcknowledgedMessage(ByteDataCarrier rawParams)
...
// ReSharper disable once UnusedMember.Local
        ...
        private bool TransmitMessage(ISimpleTcpConnection connection, LowLevelMessage lowLevelMessage)
        {
            try
            {
                ByteDataCarrier bdc = lowLevelMessage.GetRawData();
        ...
        public delegate void DIncomingTransferResult(TransferInfo transferInfo, bool success, out bool closeStreamAfterProcessing);
        ...
        /// <param name="connection"></param>
        /// <param name="data"></param>
        void OnTcpDataReceived(ISimpleTcpConnection connection, ByteDataCarrier data)
        {
            try
            {
            ...
            {
                HandledExceptionAdapter.Examine(generalError);
            }
        }
        ...
        private void ParsePullFileMessage(ISimpleTcpConnection connection, LLPullFileMessage llpf)
        {
            ...
            else
            {
                ...
                try
                {
                    FileStream fsSource = new FileStream(llpf.FilePath, FileMode.Open, FileAccess.Read, FileShare.Read);
                    if (_isCompactFramework)
                        fsSource.Flush();

                    ti = new TransferInfo(TransferSide.PullSender)
                    {
                        ...
                        _overwrite = true,
                        _overallSize = (int) fsSource.Length,
                        _storeAsFile = llpf.PullAsFile
                    };

                    _peers[ti.ToString()] = ti;

                    var sft = new SafeThread<TransferInfo, ISimpleTcpConnection>(TransferingThread);
        ...
        private void ParseStoreFileMessage(TransferInfo ti, ISimpleTcpConnection connection, LLStoreFileMessage llsf)
        {
            ...
            try
            {
                if (ti != null && ti._stream != null)
                    fs = (FileStream) ti._stream;
                else
                {
                    fs = new FileStream(llsf.FilePath,
                        FileMode.Create, FileAccess.ReadWrite, FileShare.Read, OPTIMIZED_FILE_STREAM_BUFFER_SIZE);
                    if (_isCompactFramework)
                        fs.Flush();
                }
                ...
                if (TransmitMessage(connection, llACK))
                {
                    ...
                    else
                    {
                        ...
                        if (ti.Side != TransferSide.PullInitiator)
                            DebugHelper.NOP(ti.Side);
            ...
            switch (ti._transferPhase)
            {
                case TransferPhase.InitialPushCommand:
                    ...
                    else
                    {
                        ...
                        if (ti.Side == TransferSide.PushInitiator &&
        ...
        {
            if (ti._stream.Position != ti._overallSize &&
                ...
                !ti._stopTransfer)
            {
                ...
                {
                    crc = (int) Crc32.ComputeChecksum(ti._tmpBuffer, 0, read);
                }
            ...
            else
            {
                try
                {
                    ...
                    if (llpd.CrcUsed)
                    {
                        ...
                        uint computedChecksum = Crc32.ComputeChecksum(ti._stream, byteCountToVerify);
                        //#if DEBUG
        ...
        {
            int percentage = position*100/ti._overallSize;
        ...
        private bool ResolveBinaryFragmentation(ISimpleTcpConnection connection, ByteDataCarrier data, 
            TransferInfo ti,out LowLevelMessage llm)
        {
            llm = null;
            try
            {
            ...
            {
                HandledExceptionAdapter.Examine(dataError);
            }
        ...
        void OnRemoteSideDisconnected(ISimpleTcpConnection param)
        {
            ...
            if (ti != null)
            {
                if (ti.Result <= 0)
                {
                    ...
                    if (ti.Side == TransferSide.PushReceiver)
        ...
        {
            Validator.CheckNull(source, destinationEndpoint);
            if (!source.CanRead)
        ...
        public TransferInfo Pull(
            ...
            string destinationName)
        {
            Validator.CheckNull(destinationEndpoint,sourceFileName);
        ...
        /// <param name="transferInfo"></param>
        /// <param name="success"></param>
        public delegate void DTransferResult(TransferInfo transferInfo,bool success);
        ...
        public delegate void DTransferProgress(TransferInfo transferInfo, int progress);
