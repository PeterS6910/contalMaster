...
#endif
{
    ...
    public class LevelSortedQueue<T> : ADisposable, ICollection<T> , IWaitCollection
    {
        ...
        private void IntroduceLevel(T item, uint priority, LinkedListNode<T> beforeNode)
        {
                                    ...
                                    for (int i = 0; i < _levelsSorted.Length; i++)
                                    {
                                        if (priority < _levelsSorted[i])
                                        {
                                            ...
                                            if (_levels.TryGetValue(_levelsSorted[i], out nextPriorityInfo))
                                            {
                                                ...
                                                // incosistency, should not happen
                                                Debugger.Break();
        ...
        /// <returns></returns>
        internal bool Contains(Func<T,object,bool> comparingLambda,object objectToCompareWith)
        {
            lock (_queueSyncRoot)
            {
        ...
        /// <param name="itemToCompareWith"></param>
        /// <returns></returns>
        public int Remove(Func<T,object,bool> comparingLambda, object itemToCompareWith)
        {
        ...
        private int RemoveNodes(IDictionary<LinkedListNode<T>, object> nodesToRemove)
        {
            if (nodesToRemove == null)
                return 0;
