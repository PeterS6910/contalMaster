...
namespace Contal.IwQuick.PlatformPC.UI.Accordion
{
    ...
    public class Accordion : UserControl, IMessageFilter
    {
        ...
        //--------------------------------------------
        private readonly List<Control> _resizeBarsList = new List<Control>();
        private readonly List<Control2> _control2S = new List<Control2>();
        private bool _isAnimating;
        private readonly AccordionLayoutEngine _layoutEngine = new AccordionLayoutEngine();
        private Control2 _lastChecked; // used to track the last opened control so it can be closed if OpenOneOnly is true
        ...
        public CheckBox Add(AddArgs args)
        {
            ...
            CheckBoxWithArrows checkBoxWithArrows = cbf.CreateCheckBox(
                GetUpArrow,
                GetDownArrow,
                args.text,
                check,
                Get(checkboxMargin, CheckBoxMargin));
            ...
            Control2 c2 = new Control2(checkBoxWithArrows, c, resizeBar, fillWt, layoutArgs);
            ...
            {
                bool b = checkBoxWithArrows.Checked;
                if (b)
                {
                    c2.lastClicked = DateTime.Now;
                    if (OpenOneOnly && _lastChecked != null && c2 != _lastChecked)
                    {
                        _isAdjusting = true;
                        _lastChecked.cb.Checked = false;
                        _isAdjusting = false;
                    }
                    _lastChecked = c2;
                }
                ...
                if (VerticalScroll.Visible)
                {
                    ...
                    if (checkBoxWithArrows.Checked)
                    {
                        ...
                        if (r1.Bottom > r2.Bottom)
                        {
                            ...
                            ScrollControlIntoView(checkBoxWithArrows);
                            isOpening = false;
                        }
                    }
                    else
                    {
                        ...
                        GetWindowRect(checkBoxWithArrows.Handle, out r1);
            ...
            checkBoxWithArrows.CheckedChanged += delegate
            {
                bool b = checkBoxWithArrows.Checked;
                ...
                checkBoxWithArrows.RefreshText();
                _isAnimating = false;
                ...
                if (b && AnimateOpenEffect != AnimateWindowFlags.None && AnimateOpenMillis > 0)
                {
                    ...
                    if (holdSpace)
                    {
                        ...
                        if (r1.Bottom > r2.Bottom)
                        {
                            mOrig = checkBoxWithArrows.Margin;
                            Padding m = mOrig;
                            m.Bottom += c2.Height + c2.Margin.Vertical;
                            checkBoxWithArrows.Margin = m;
                    ...
                    if (holdSpace)
                        checkBoxWithArrows.Margin = mOrig; // put back original margin
                ...
                {
                    _isAnimating = true; // prevent c2.Visible from triggering PerformLayout
            ...
            checkBoxWithArrows.VisibleChanged += delegate
            {
                bool b = checkBoxWithArrows.Visible && checkBoxWithArrows.Checked;
                if (c2.Visible != b)
                {
                    _isAdjusting = true;
                    c2.Visible = b;
            ...
            checkBoxWithArrows.CheckedChanged += cb_CheckedChanged;
            return checkBoxWithArrows;
        }

        //private bool _isBlockedChbTextChanged;

        public event Action<bool?> OpenedAllChnged;

        void cb_CheckedChanged(object sender, EventArgs e)
        {
            if (OpenedAllChnged == null)
                return;

            OpenedAllChnged(IsOpenAll);
        }

        public void CloseAll()
        {
            var controls = new LinkedList<Control>(
                _control2S.Select(control2 => control2.c));

            Close(controls.ToArray());
        }
        ...
        public bool? IsOpenAll
        {
            get
            {
                var checkBoxes = new LinkedList<CheckBox>(
                    _control2S.Select(control2 => control2.cb));

                bool? isExpandAll = null;

                foreach (var checkBox in checkBoxes)
                {
                    if (isExpandAll == null)
                    {
                        isExpandAll = checkBox.Checked;
                        continue;
                    }

                    if (isExpandAll.Value != checkBox.Checked)
                    {
                        isExpandAll = null;
                        break;
                    }
        ...
        public IEnumerable<Control> ResizeBars
        {
            get
            {
                foreach (Control c in _resizeBarsList)
                    yield return c;
            }
        }
        ...
        private void Open(Control[] controls, bool open)
        {
            ...
            bool changed = false;
            Control2 last = null;
            foreach (Control2 c2 in _control2S)
            {
                Control2 c = null;
                if (controls == null)
                    c = c2;
        ...
        public void OpenAndScrollToControl(Control control)
        {
            if (control == null)
                return;
            ...
            var control2 = control.Parent as Control2;

            if (control2 == null)
                return;

            var control2Top = control2.GetTop();

            if (control2Top < 0)
            {
                VerticalScroll.Value += control2Top;
            }

            var control2Bottom = control2.Bottom;

            if (control2Bottom > Height)
            {
                VerticalScroll.Value += control2Bottom - Height;
            }
        }
            ...
            if (ResizeBarsFadeProximity > 0)
            {
                ...
                if (!r.Contains(pt))
                {
                    if (ResizeBarsFadeProximity > 0)
                    {
                        ...
                        foreach (Control rb in _resizeBarsList)
                        {
                            if (rb.Focused)
                            {
        ...
        internal bool IsResizeBar(Control c)
        {
            if (c == null)
                return false;
            return _resizeBarsList.Contains(c);
        }

        public override Size GetPreferredSize(Size proposedSize)
        {
            bool includeInvisible = !this.Visible;
            return _layoutEngine.GetPreferredSize(this, true, true, includeInvisible);
        }
        ...
        {
            foreach (var control2 in _control2S)
            {
                control2.cb.Enabled = enabled;
            }
        }
        ...
        internal class AccordionLayoutEngine : LayoutEngine
        {
            ...
            public override bool Layout(Object container, LayoutEventArgs layoutEventArgs)
            {
                ...
                if (acc._isAnimating || acc.IsResizeBar(layoutEventArgs.AffectedControl))
                    return false;
            ...
            private Size LayoutInternal(bool doLayout, Accordion acc, int offsetX, int offsetY, Size avail, Padding p, bool calcPreferredSize, Size clientSize, bool addDH, bool returnEmptyIfClosed, bool includeInvisible)
            {
                ...
                if (doLayout)
                {
                    ...
                    foreach (Control2 c2 in acc._control2S)
                    {
                        if (c2.Visible)
            ...
            private void UpdateDeltaHeights(Accordion acc, int clientSizeHeight, int prefHeightNoDH)
            {
                ...
                else
                {
                    if (eh < 0 && totalWt > 0)
                    {
                        ...
                        bool[] isLocked = new bool[acc._control2S.Count];
                        int[] minDHs = new int[acc._control2S.Count];
                        int mh2 = 0;
                        bool first = true;
                        do
                        {
                            mh2 = 0;
                            double pixels2 = 0;
                            for (int i = 0; i < acc._control2S.Count; i++)
                            {
                                Control2 c2 = acc._control2S[i];
                                if (c2.isLocked)
                                    isLocked[i] = true;
                    ...
                    else if (eh > 0 && totalWt > 0)
                    {
                        ...
                        bool[] isLocked = new bool[acc._control2S.Count];
                        int[] maxDHs = new int[acc._control2S.Count];
                        int mh2 = 0;
                        bool first = true;
                        do
                        {
        ...
        private class ToolBox : ToolStripDropDown
        {
            ...
            public ToolBox()
            {
                ...
                Action<Object> fillAction = (src) =>
                {
                    ...
                    Size ps = a._layoutEngine.GetPreferredSize(a, true, true, false);
                    ...
                    if (src == miFill)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (Control2 c2 in a._control2S)
                            {
                                if (c2.Visible)
                                {
                                    numControl2Visible++;
                ...
                Action<Object> packAction = (src) =>
                {
                    ...
                    {
                        foreach (Control2 c2 in host._control2S)
                        {
                            c2.Tag = c2.isLocked;
                            c2.isLocked = true;
        ...
        {
            CheckBoxWithArrows CreateCheckBox(
                Func<string> getUpArrow,
                Func<string> getDownArrow,
                string text,
                bool check,
                Padding margin);
        }
        ...
        {
            public virtual CheckBoxWithArrows CreateCheckBox(
                Func<string> getUpArrow,
                Func<string> getDownArrow,
                string text,
                bool check,
                Padding margin)
            {
                return new CheckBoxWithArrows(
                    getUpArrow,
                    getDownArrow)
                {
                    Appearance = Appearance.Button,
                    //AutoSize = true, // AutoSize must be true or the text will wrap under and be hidden
                    Checked = check,
                    Anchor = AnchorStyles.Left | AnchorStyles.Right, // cb.Dock = DockStyle.Fill also works.
                    Margin = margin, // typically 0 so that no are gaps between the buttons
                    AutoEllipsis = true,
                    Text = text
                };
            }
    ...
    public class CheckBoxWithArrows : CheckBox
    {
        private string _text;
        private readonly Func<string> _getUpArrow;
        private readonly Func<string> _getDownArrow;

        public CheckBoxWithArrows(
            Func<string> getUpArrow,
            Func<string> getDowArrow)
        {
            _getUpArrow = getUpArrow;
            _getDownArrow = getDowArrow;
        }
        ...
        {
            var upArrow = _getUpArrow != null
                ? _getUpArrow()
                : string.Empty;

            var downArrow = _getDownArrow != null
                ? _getDownArrow()
                : string.Empty;
