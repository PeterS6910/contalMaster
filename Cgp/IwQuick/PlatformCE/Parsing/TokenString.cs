...
namespace Contal.IwQuick.Parsing
{
    public class TokenString
    {
        ...
        public void SetCaseSensitive(bool i_bInput)
        {
            _caseSens = i_bInput;
        }
        ...
        public static string Combine(
            [NotNull] Array elements,
            [NotNull] string separator)
        {
            Validator.CheckForNull(elements, "elements");
            Validator.CheckForNull(separator,"separator");

            StringBuilder ret = new StringBuilder(elements.Length * 20);
            string subVal;
            foreach (object val in elements)
            {
                try
                {
                    subVal = val.ToString();
