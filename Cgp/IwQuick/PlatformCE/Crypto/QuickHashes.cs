using System;
using System.Text;

using System.Security.Cryptography;
using Contal.IwQuick.Data;

namespace Contal.IwQuick.Crypto
{
    /// <summary>
    /// 
    /// </summary>
    public class QuickHashes
    {
        private static readonly SHA1 _sha1 = new SHA1CryptoServiceProvider();

        /// <summary>
        /// computes SHA1 hash and returns in form of hexadecimal formatted string
        /// </summary>
        /// <param name="input"></param>
        /// <param name="encoding"></param>
        /// <returns></returns>
        public static string GetSHA1String(string input, Encoding encoding)
        {
            if (null == encoding)
                return String.Empty;
            
            if (null == input)
                return String.Empty;

            string retString = null;
            ByteDataCarrier.StringToBytes(input,encoding, (stringBytes, stringBytesLength) =>
            {
                retString = GetSHA1String(stringBytes, 0, stringBytesLength);
            });

            return retString;

        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetSHA1String(string input)
        {
            return GetSHA1String(input, Encoding.UTF8);
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <param name="offset"></param>
        /// <param name="length"></param>
        /// <returns></returns>
        public static string GetSHA1String(byte[] input,int offset,int length)
        {
            if (null == input || offset < 0 || length < 0)
                return string.Empty;

            if (offset >= input.Length)
                offset = 0;

            if (offset + length > input.Length)
            {
                length = input.Length - offset;
            }

            try
            {

                byte[] output;
                lock (_sha1)
                {
                    output = _sha1.ComputeHash(input, offset, length);
                }

                var sb = StringBuilderPool.Implicit2k.Get();
                try
                {
                    for (int i = 0; i < output.Length; i++)
                        sb.Append(output[i].ToString(StringConstants.HEX2_FORMAT).ToLower());

                    return sb.ToString();
                }
                finally
                {
                    if (sb !=null)
                        StringBuilderPool.Implicit2k.Return(sb);
                }

            }
            catch
            {
                return String.Empty;
            }
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetSHA1String(byte[] input)
        {
            return GetSHA1String(input, 0 ,input.Length);
        }


        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetSHA512String(string input)
        {
            return null;
            /*
            if (null == i_strValue)
                i_strValue = String.Empty;

            
            SHA512Managed aSha = new SHA512Managed();

            byte[] arData = Encoding.ASCII.GetBytes(i_strValue);
            arData = aSha.ComputeHash(arData);

            StringBuilder strRet = new StringBuilder(String.Empty, 32);
            for (int i = 0; i < arData.Length; i++)
                strRet.Append(arData[i].ToString(StringConstants.HEX2_FORMAT).ToLower());

            return strRet.ToString();
            */
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetSHA256String(string input)
        {
            return null;
            /*
            if (null == i_strValue)
                i_strValue = String.Empty;


            SHA256 aSha = new SHA256Managed();

            byte[] arData = Encoding.ASCII.GetBytes(i_strValue);
            arData = aSha.ComputeHash(arData);

            StringBuilder strRet = new StringBuilder(String.Empty, 32);
            for (int i = 0; i < arData.Length; i++)
                strRet.Append(arData[i].ToString(StringConstants.HEX2_FORMAT).ToLower());

            return strRet.ToString();
            */
        }

        //private static void CountSHA256(string inputValue)
        //{
        //    //Note 1: All variables are unsigned 32 bits and wrap modulo 232 when calculating
        //    //Note 2: All constants in this pseudo code are in big endian

        //    //Initialize variables
        //    //(first 32 bits of the fractional parts of the square roots of the first 8 primes 2..19):

        //    UInt32 h0 = 0x6a09e667;
        //    UInt32 h1 = 0xbb67ae85;
        //    UInt32 h2 = 0x3c6ef372;
        //    UInt32 h3 = 0xa54ff53a;
        //    UInt32 h4 = 0x510e527f;
        //    UInt32 h5 = 0x9b05688c;
        //    UInt32 h6 = 0x1f83d9ab;
        //    UInt32 h7 = 0x5be0cd19;

        //    //Initialize table of round constants
        //    //(first 32 bits of the fractional parts of the cube roots of the first 64 primes 2..311):
        //    UInt32[] k = new UInt32[] {
        //       0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        //       0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        //       0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        //       0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        //       0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        //       0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        //       0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        //       0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2 };


        //    byte[] message = Encoding.Unicode.GetBytes(inputValue);
        //    int length = message.Length * sizeof(byte);

        //}

        private static readonly MD5 _md5 = new MD5CryptoServiceProvider();
        
        /// <summary>
        /// quick method for 32-character MD5 checksum
        /// </summary>
        /// <param name="input">string input</param>
        /// <returns>32-character hex-presented MD5 checksum</returns>
        public static string GetMD5String(string input)
        {
            if (null == input)
                input = String.Empty;
            
            byte[] arData = Encoding.ASCII.GetBytes(input);

            lock (_md5)
            {
                arData = _md5.ComputeHash(arData);
            }

            var sb = StringBuilderPool.Implicit2k.Get();
            for (int i = 0; i < arData.Length; i++)
                sb.Append(arData[i].ToString(StringConstants.HEX2_FORMAT).ToLower());

            var returnString = sb.ToString();

            StringBuilderPool.Implicit2k.Return(sb);

            return returnString;
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetCRC32String(string input)
        {
            if (null == input)
                input = String.Empty;

            byte[] checksumBytes = Crc32.ComputeChecksumBytes(input);

            var sb = StringBuilderPool.Implicit2k.Get();
            for (int i = 0; i < checksumBytes.Length; i++)
                sb.Append(checksumBytes[i].ToString(StringConstants.HEX2_FORMAT).ToLower());

            var returnString = sb.ToString();

            StringBuilderPool.Implicit2k.Return(sb);

            return returnString;
        }
    }
}
