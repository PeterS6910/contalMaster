...
namespace Contal.IwQuick.Net
{
    ...
    public class SslHelper2 : IDisposable
    {
        ...
        private IntPtr _ptrHost;

        private IntPtr _certificateValidationHook;

        private SslSettings _sslSettings;


        private byte[] _sslSetValidateCertHook_parameters;
        public SslHelper2(
            [NotNull] Socket socket, 
            [NotNull] SslSettings sslSettings)
        {
            Validator.CheckForNull(socket, "socket");
            Validator.CheckForNull(sslSettings, "sslSettings");
            ...
            _certificateValidationHook = Marshal.GetFunctionPointerForDelegate(new SSLVALIDATECERTFUNC(ValidateCert));

            //Allocate the buffer for the string

            _ptrHost = Marshal.AllocHGlobal(_sslSettings.TargetHost.Length + 1);
            GC.SuppressFinalize(_ptrHost);

            WriteASCIIString(_ptrHost, _sslSettings.TargetHost);

            //Now put both pointers into a byte[]

            _sslSetValidateCertHook_parameters = new byte[8];
            byte[] hookFuncBytes = BitConverter.GetBytes(_certificateValidationHook.ToInt32());
            byte[] hostPtrBytes = BitConverter.GetBytes(_ptrHost.ToInt32());
            
            Array.Copy(hookFuncBytes, _sslSetValidateCertHook_parameters, hookFuncBytes.Length);
            Array.Copy(hostPtrBytes, 0, _sslSetValidateCertHook_parameters, hookFuncBytes.Length, hostPtrBytes.Length);

            GC.SuppressFinalize(_sslSetValidateCertHook_parameters);

            unchecked
            {
                socket.IOControl((int)SO_SSL_SET_VALIDATE_CERT_HOOK, _sslSetValidateCertHook_parameters, null);
            }
        }
        ...
        private void ReleaseHostPointer()
        {
            ...
            if (_ptrHost != IntPtr.Zero)
            {

                Marshal.FreeHGlobal(_ptrHost);

                _ptrHost = IntPtr.Zero;
        ...
        private int ValidateCert(uint dwType, IntPtr pvArg, uint dwChainLen, IntPtr pCertChain, uint dwFlags)
        {
            ...
            if (!_sslSettings.AllowUntrustedRoot &&
                (dwFlags & SSL_CERT_FLAG_ISSUER_UNKNOWN) != 0)
            ...
            if (!_sslSettings.AllowExpiredOrIneffectiveCertificate)
            {
            ...
            if (Validator.IsNotNullString(_sslSettings.TargetHost) &&
                !_sslSettings.AllowCertificateNameMismatch)
            {
